...
for i in range(1, 51):
    ...
    temp_circle = np.load('data/circle' + str(i) + '.npy')
    temp_static = np.load('data/static' + str(i) + '.npy')

    # CSI求模&&毛刺处理
    for j in range(PCAP_SIZE):
        for k in range(NFFT):
            temp_clap[j][k] = abs(temp_clap[j][k])
            temp_circle[j][k] = abs(temp_circle[j][k])
            temp_static[j][k] = abs(temp_static[j][k])
    ...
    temp_circle = temp_circle.astype('float64')
    temp_static = temp_static.astype('float64')
    train_data[i - 1] = temp_clap[0:PCAP_SIZE, :]
    train_data[50 + i - 1] = temp_circle[0:PCAP_SIZE, :]
    train_data[100 + i - 1] = temp_static[0:PCAP_SIZE, :]
    train_result[i - 1] = 0
